...
def render(painter, **args):
	...
	# lower right
	xy1 = project_point([L, heightGap, 0], painter, args)
	# upper right
	xy2 = project_point([L, L + heightGap, 0], painter, args)
	# lower left
	xy3 = project_point([0.0, heightGap, 0], painter, args)
	# upper left
	xy4 = project_point([0.0, L + heightGap, 0], painter, args)
	
	gapSize = 1.2
	gapSides = (L-gapSize)/2
	# gap left
	xy5 = project_point([gapSides, heightGap, 0], painter, args)
	# gap right
	xy6 = project_point([gapSides+gapSize, heightGap, 0], painter, args)
	
	# lower left line
	xy7 = project_point([0.0, 0.0, 0], painter, args)
	# lower right line
	xy8 = project_point([L, 0.0, 0], painter, args)
	...
	painter.drawLine(xy3[0],xy3[1],xy7[0],xy7[1])
	painter.drawLine(xy1[0],xy1[1],xy8[0],xy8[1])
